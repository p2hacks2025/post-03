...
- ユーザーの悩み：自分の声をスマホで録音して聴くと、平坦でショボく聞こえてしまい、憧れのアーティストとの差に絶望している。本格的な「歌ってみた」を投稿したいが、数万円するマイクやオーディオインターフェースなどの機材を買うお金がない。従来の編集ソフトは難しすぎて、ピッチ補正やエコーなどの加工が自分ではできない。
...
- アプリを起動し音声ファイルを選択をタップ、自分のスマホからエフェクトをかけたいmp3、wavファイルを選択する。次かけたいエフェクトを選択し実行を実行をタップする。
